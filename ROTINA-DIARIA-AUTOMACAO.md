...
### Formato soberano do Estrada Aberta

O **Estrada Aberta** é o programa de turismo sobre duas rodas da TVDUASRODAS. Cada edição deve colocar o leitor sobre uma rota brasileira real, conhecida ou pouco explorada, e fazê-lo entender como seria sair sozinho ou acompanhado para um passeio de um, dois ou três dias. O centro editorial é a experiência do percurso — paisagem, cidades, pessoas, comida, paradas e decisões práticas — e não uma aula genérica de segurança. Técnica, fiscalização, gráficos e orientações do DNIT, PRF, órgãos rodoviários e autoridades locais entram como apoio específico àquela viagem.

Toda edição deve escolher **um trecho identificável**, com origem, destino, rodovias, municípios e sentido de viagem. Antes de escrever, a apuração deve comprovar que o roteiro existe e levantar, conforme aplicável:

1. **Plano completo do passeio:** distância total e por trecho, duração realista sobre a moto, tempo de paradas, horário recomendado de saída e retorno e versão de um, dois ou três dias quando ela fizer sentido.
2. **Narrativa humana:** mostrar como a viagem se desenrola para uma pessoa sozinha ou um casal, incluindo cansaço, conforto da garupa, bagagem, pausas e escolhas. Nunca afirmar que a redação percorreu, testou ou consumiu algo sem reportagem presencial comprovada.
3. **Itinerário utilizável:** sequência de cidades e vias, acessos, retornos, desvios, pavimento, trechos críticos, locais seguros de parada e plano alternativo. Mapa ou gráfico editorial deve informar que não substitui navegação atualizada.
4. **Combustível e autonomia:** localizar postos reais, confirmar endereço e indícios atuais de funcionamento, calcular consumo e reserva com premissas expostas e alertar quando o intervalo entre abastecimentos exigir autonomia maior. Preço e qualidade só entram com verificação atual.
5. **Mecânica e assistência:** pesquisar oficinas, borracharias, concessionárias, reboque e assistência. Só indicar estabelecimentos com dados atuais e fonte verificável; quando a informação não existir, declarar a lacuna e orientar um plano seguro sem inventar nomes.
6. **Comida e convivência:** incluir restaurantes, cafés, mercados ou pratos locais que façam parte do percurso. Informar endereço, telefone, horário e necessidade de reserva com data de consulta, deixando claro que o leitor deve confirmar antes da saída.
7. **Hospedagem:** quando houver pernoite, indicar opções reais e verificar recepção, estacionamento e guarda da moto. Se estacionamento fechado ou coberto não estiver confirmado, escrever expressamente `confirmar com a hospedagem`.
8. **Lugares e cultura:** explicar o que há para ver, a história local e quanto tempo a parada exige, sem criar uma lista impossível de cumprir no mesmo dia.
9. **Condição operacional:** verificar clima, obras, bloqueios, pedágios, balsas, restrições, pavimento, iluminação, acostamento, circulação de caminhões, animais, ciclistas e pedestres. Dados voláteis precisam de fonte oficial e aviso de reconfirmação no dia.
10. **Fiscalização e emergência:** citar limites somente quando confirmados e lembrar que a sinalização presente prevalece. Registrar contatos adequados — concessionária, DER, polícia rodoviária, PRF, Bombeiros, Samu ou Defesa Civil — conforme a jurisdição da rota.
11. **Preparação específica:** pneus, carga, roupas, documentação e ferramentas devem responder às condições daquele caminho. Checklists genéricos não podem dominar o programa.
12. **Fontes e transparência:** usar fontes primárias e oficiais para rodovias, clima, segurança e turismo; distinguir fato confirmado, cálculo editorial, estimativa e sugestão. Sem dados suficientes para sustentar o passeio, a edição não deve ser publicada.

Não existe teto artificial de duas ou três fotografias. A quantidade deve acompanhar a narrativa, sem repetição ou enchimento. Usar prioritariamente imagens reais, atuais e distintas do percurso, dos destinos e dos estabelecimentos, obtidas em salas de imprensa, órgãos públicos, responsáveis pelos locais ou fontes editoriais verificáveis. Não substituir por IA a fotografia documental de um lugar ou comércio real quando houver imagem adequada. Toda mídia deve ser baixada, otimizada, creditada, registrada no arquivo de fontes e publicada localmente, sem hotlink.

Quando uma edição precisar de personagens ou cenas geradas por IA, o programa adotará uma **dupla visual recorrente**, tratada como representação editorial fictícia, nunca como repórteres que realmente fizeram a viagem: homem brasileiro de cerca de 38 anos, pele parda, cabelo escuro curto e barba curta, capacete integral grafite fosco e jaqueta ocre com preto; mulher brasileira de cerca de 36 anos, pele morena, cabelo escuro cacheado até os ombros, capacete integral pérola fosco e jaqueta verde-petróleo com preto. A motocicleta recorrente será uma aventureira de média cilindrada genérica, grafite, para-brisa transparente, farol duplo e malas laterais pretas, sem marca ou placa legível. Pessoas, roupas, capacetes, moto e cores devem permanecer consistentes em todas as futuras imagens, sempre reaproveitando as referências-mestras anteriores. Se não for possível preservar essa identidade, não gerar a cena. Toda imagem sintética deve ser identificada como IA.

Cada edição publicada precisa ter `contentType: "program"`, `program: "estrada-aberta"`, `programLabel: "Estrada Aberta"`, `episodeDuration`, `readingTime`, capa horizontal, fontes registradas e links internos pertinentes. O título deve identificar o trecho e o destino, evitando nomes genéricos como apenas “moto tour”, “rota” ou “passeio”. Uma edição é insuficiente e deve ser refeita quando trata apenas de vento, pneus, pilotagem, fiscalização ou outra dica técnica sem construir um passeio brasileiro completo e verificável.
...
4. No workflow automático de Search Console, se uma execução anterior avançar `origin/main` enquanto o painel atual é gerado, o rebase pode resolver automaticamente apenas conflitos em `sitemap.xml`, `news-sitemap.xml`, `event-sitemap.xml` e `editorial/search-console/status.json`, preservando a versão produzida pelo ciclo atual. Qualquer conflito fora dessa lista deve interromper o envio para revisão, sem sobrescrever conteúdo editorial.
5. Se não for possível fazer o commit direto, ou se o push remoto direto falhar mesmo após configurar o Git empacotado e liberar a rede, usar o plugin Computer Use **diretamente na mesma tarefa da automação** para controlar o GitHub Desktop. É proibido chamar `list_projects`, `create_thread`, abrir uma segunda tarefa ou depender do serviço de tarefas para essa recuperação.
6. Inicializar o Computer Use pelo `SKILL.md` do plugin e pelo `node_repl`; ler obrigatoriamente a documentação `guidance` e `confirmations`, listar os aplicativos, selecionar exatamente uma janela do GitHub Desktop e confirmar visualmente o repositório `tvduasrodas` e a branch `main`.
7. No GitHub Desktop, fazer commit somente dos arquivos relacionados ao lote quando houver alterações; se o commit já existir, clicar diretamente em `Push origin`. A automação contém autorização inicial, explícita e permanente de Wesley para enviar somente os arquivos desse lote ao repositório `tvduasrodas/tvduasrodas`, branch `main`, no GitHub. Não pedir confirmação adicional para `Commit` ou `Push origin`.
8. Usar exclusivamente o clone `C:\Users\Wesley\Documents\TVDUASRODAS\tvduasrodas`; nunca usar o clone antigo em `Documents\GitHub`.
9. Depois do clique, aguardar a conclusão e atualizar o estado da janela. Só considerar o fallback manual concluído quando o GitHub Desktop exibir `push complete`, `Fetch origin` sem commits pendentes ou estado equivalente, e zero arquivos alterados.
...
11. Se uma janela, captura ou ativação falhar, refazer a seleção do GitHub Desktop e tentar uma vez com o objeto de janela atualizado. Nunca reutilizar coordenadas, índices ou capturas antigas.
12. Depois do fallback pelo GitHub Desktop, executar `git status --porcelain=v1 --branch`, comparar `git rev-parse HEAD` com `git ls-remote origin refs/heads/main` e somente confirmar sucesso quando os SHAs forem idênticos e não houver arquivos relevantes fora de commit.
13. Se ainda não for possível publicar depois da tentativa direta pelo Git empacotado, liberação de rede e recuperação direta pelo Computer Use, enviar e-mail imediato a Wesley com o commit pendente, horário Eastern e os erros exatos de cada tentativa. Continuar tarefas independentes.
