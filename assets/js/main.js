...
// ==== CONFIG GITHUB PARA MATÉRIAS ====
const GITHUB_OWNER = "tvduasrodas";      // ajuste se o repo for outro
const GITHUB_REPO = "tvduasrodas";       // idem
const GITHUB_BRANCH = "main";
const ARTICLES_PATH = "content/articles";

// Monta a URL RAW do JSON da matéria
function buildArticleUrl(slug) {
    return `https://raw.githubusercontent.com/${GITHUB_OWNER}/${GITHUB_REPO}/${GITHUB_BRANCH}/${ARTICLES_PATH}/${slug}.json`;
}

// Pega o slug da URL: materia.html?slug=review-naked-300
function getArticleSlugFromUrl() {
    const params = new URLSearchParams(window.location.search);
    return params.get("slug");
}

// Formata data tipo "23 jan 2026"
function formatArticleDate(dateStr) {
    ...
    return `${day} ${months[monthStr] || monthStr} ${year}`;
}
...
document.addEventListener("DOMContentLoaded", () => {
   ...
   ============================ */
    ...
    async function loadArticlePage() {
        // Garante que só rode em páginas que têm o layout de matéria
        const bodyEl = document.getElementById("articleBody");
        if (!bodyEl) return;

        const titleEl = document.getElementById("articleTitle");
        const breadcrumbTagEl = document.getElementById("articleBreadcrumbTag");
        ...
        const sponsorWrapperEl = document.getElementById("articleSponsorWrapper");
        const sponsorEl = document.getElementById("articleSponsor");

        const heroVideoEl = document.getElementById("articleHeroVideo");
        const heroIframeEl = document.getElementById("articleHeroIframe");
        const heroImageEl = document.getElementById("articleHeroImage");
        const heroImageTagEl = document.getElementById("articleHeroImageTag");
        const heroCaptionEl = document.getElementById("articleHeroCaption");
        const videoCtaContainerEl = document.getElementById("articleVideoCtaContainer");

        const shareFbEl = document.getElementById("shareFb");
        const shareTwEl = document.getElementById("shareTw");
        const shareWaEl = document.getElementById("shareWa");

        const slug = getArticleSlugFromUrl();

        if (!slug) {
            titleEl.textContent = "Matéria não encontrada";
            bodyEl.innerHTML = "<p>Slug da matéria não foi informado na URL.</p>";
            return;
        }
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error("Erro ao carregar JSON da matéria: " + res.status);
            }

            const article = await res.json();

            // Mapeando campos do JSON (ajusta se os nomes forem diferentes)
            const title = article.title || "Matéria sem título";
            const category = article.category || "Revista";
            const kicker = article.kicker || article.tagline || "";
            const author = article.author || "Redação TV Duas Rodas";
            const dateStr = formatArticleDate(article.date);
            const readingTime = article.readingTime || "";
            const sponsor = article.sponsor || "";
            const bodyHtml = article.bodyHtml || "<p>Sem conteúdo.</p>";
            const hero = article.hero || null;
            const videoId = article.videoId || "";
            const summary = article.summary || "";

            // Título / meta
            titleEl.textContent = title;
            document.title = `${title} | TV Duas Rodas`; // atualiza <title> da aba

            // Breadcrumb + tag
            breadcrumbTagEl.textContent = category;
            tagEl.textContent = kicker
                ? `${kicker} · ${category}`
                : category;
            ...
            if (sponsor) {
                ...
                sponsorWrapperEl.hidden = false;
            } else {
                sponsorWrapperEl.hidden = true;
            }

            // Hero: se tiver videoId, prioriza vídeo
            heroVideoEl.hidden = true;
            heroImageEl.hidden = true;
            videoCtaContainerEl.innerHTML = "";

            if (videoId) {
                const embedUrl = `https://www.youtube.com/embed/${videoId}`;
                heroIframeEl.src = embedUrl;
                heroIframeEl.title = title;
                heroVideoEl.hidden = false;

                // Botão "Assistir na TV & Vídeos"
                videoCtaContainerEl.innerHTML = `
        <a href="tv.html?v=${encodeURIComponent(videoId)}"
           class="btn btn-outline btn-small">
          Assistir este episódio na TV &amp; Vídeos &rarr;
        </a>
      `;
            } else if (hero && hero.type === "image" && hero.image) {
                const imgSrc = hero.image.startsWith("http")
                    ? hero.image
                    : hero.image.startsWith("/")
                        ? hero.image
                        : "/" + hero.image.replace(/^\.?\//, "");

                heroImageTagEl.src = imgSrc;
                heroImageTagEl.alt = hero.alt || title;
                heroCaptionEl.textContent = hero.caption || "";
                heroImageEl.hidden = false;
            }

            // Corpo da matéria (HTML vindo do CMS)
            bodyEl.innerHTML = bodyHtml;

            // Links de compartilhamento
            const pageUrl = window.location.href;
            const shareText = `${title} - TV Duas Rodas`;
            ...
            console.error(err);
            titleEl.textContent = "Erro ao carregar matéria";
            bodyEl.innerHTML =
                "<p>Não foi possível carregar esta matéria agora. Tente novamente mais tarde.</p>";
        }
    }
